var levels = [
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p>到现在为止，你已经将你的花园设置为5列，每列占总宽度的20%；5行，每行占全部高度的20%。</p><p>这是通过设置<code>grid-template-columns: 20% 20% 20% 20% 20%;</code>和<code>grid-template-rows: 20% 20% 20% 20% 20%;</code>来实现的。每一条规则都有5个值，代表创建了5个列，每一列设置为花园宽度的20%。</p><p>但是你也可以任意设置网格项的宽度。为<code>grid-template-columns</code>设置一个新的值来给你的胡萝卜浇水吧。你一定会将你的第一列的宽度设置为50%的。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p>指定一些具有相同的宽度的网格项会变得很乏味。幸运的是有<code>repeat</code>函数来帮助我们。</p><p>比如说，之前我们使用<code>grid-template-columns: 20% 20% 20% 20% 20%;</code>属性定义了5列，每列占20%。这可以被简写为：<code>grid-template-columns: repeat(5, 20%);</code></p><p>使用<code>grid-template-columns</code>属性以及<code>repeat</code>函数，创建8列，每列占12.5%的宽度。这样你也不会给花园浇过多的水。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p><code>grid-template-columns</code>不仅仅只接受百分比的值，也接受像像素或ems这样的长度单位。你甚至可以将不同的长度单位混合使用。</p><p>接下来，分别将列的宽度设置为<code>100px</code>， <code>3em</code>，和<code>40%</code>。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p>网格系统也引入了一个新的单位，分数<code>fr</code>。每一个<code>fr</code>单元分配一个可用的空间。比如说，如果两个元素分别被设置为<code>1fr</code>和<code>3fr</code>， 那么空间就会被平均分配为4份；第一个元素占据 1/4 ，第二个元素占据3/4，也就会说剩下所有的空间。</p><p>这里，杂草占据了你第一行的左1/6，胡萝卜占据着剩下的5/6。创建两个列，并使用<code>fr</code>单位来定义它们的宽度。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p>当列的宽度采用像素，百分比或者ems的单位的时候，其他使用<code>fr</code>单位设置的列将会平分剩下的空间。</p><p>这里的胡萝卜在左边形成了一个50像素的列，并且杂草在右边也占据着50像素的宽度。使用<code>grid-template-columns</code>属性，创建这两个列，使用<code>fr</code>再创建三列，使其平均占据剩下的空间。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p><code>grid-template-rows</code>和<code>grid-template-columns</code>的作用大概相同。</p><p>用<code>grid-template-rows</code>属性来为花园的上50像素的高度浇水。注意：水被设置为仅填充第五行，所以你总共需要创建5行。</p>',
  ...
  },
  {
    ...
    instructions: {
      ...
      'zh-cn': '<p>你的花园来起来很棒。现在，你在花院的底部留下了50像素的小路，其他的空间用来种植胡萝卜。</p><p>不幸的是，胡萝卜地的20%已经杂草丛生了，最后一次用CSS 网格布局来规划你的花园吧！</p>',
...
var levelWin = {
  ...
  instructions: {
    ...
    'zh-cn': '<p>恭喜你赢得了游戏！ 通过CSS网格的力量，你能够生长足够的胡萝卜给青蛙，让它用来烘焙世界闻名的胡萝卜蛋糕。什么？你期待一个不同的朋友么？</p><p>如果你喜欢网格花园，一点也要查看<a href="http://flexboxfroggy.com/">Flexbox 青蛙</a>哦，你可以学习另外一种强大的CSS布局方式。你也可以随时查看我的最新<a href="http://thomaspark.co">博客</a>和<a href="https://twitter.com/thomashpark">Twitter</a>上的项目。</p><p>想要支持网格花园么？快来体验下<a href="http://treehouse.7eer.net/c/371033/228915/3944?subId1=gridgarden">Treehouse</a>提供和的topnotch网页设计和编码的课程。最后，也欢迎您向您的朋友和家人推荐网格花园！感谢！</p>',
